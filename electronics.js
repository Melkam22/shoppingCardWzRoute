/* search-bar */
/* function myFunction(){
  let input, filter, section, list-product, div, prod-title;
  input.document.getElementById("myInput");
  filter = input.value.toUpperCase();
  section = document.getElementById("list-product");
  div = section.document.getElementByTagName("div");
  for(a=0; a<div.length; a++){
    prod-title = div[a].getElementsByTagName("prod-title")[0];
    if(prod-title.innerHTML.toUpperCase().indexOf(filter) > -1){
      div[a].style.display = ""
    } else{
...
/* let displayFrame = document.querySelector(".frameDetail");
let btnHideShow = document.querySelector(".hideShow");
let bigScreen = true;
...
let closeBox = document.querySelector(".deleteShow");
...
function updatePrice() {
  alert("purchase electronics clicked");
